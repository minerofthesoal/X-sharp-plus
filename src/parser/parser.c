...
static void error_at(Parser* p, Token* tok, const char* msg) {
    ...
    if (p->error_count < PARSER_MAX_ERRORS) {
        snprintf(p->errors[p->error_count], PARSER_MAX_ERROR, "[line %d:%d] Error at '%.*s': %s",
...
static AstNode* parse_primary(Parser* p) {
    ...
    if (match(p, TOK_SPELL)) {
        ...
        while (!check(p, TOK_RBRACE) && !is_at_end(p)) {
            ...
            if (stmt)
                ast_node_list_push(&body->as.block.statements, stmt);
        }
...
static AstNode* parse_block(Parser* p) {
    ...
    while (!check(p, TOK_RBRACE) && !is_at_end(p)) {
        ...
        if (decl)
            ast_node_list_push(&node->as.block.statements, decl);
    }
...
static AstNode* parse_statement(Parser* p) {
    if (match(p, TOK_ORACLE))
        return parse_oracle_stmt(p);
    if (match(p, TOK_CYCLE))
        return parse_cycle_stmt(p);
    if (match(p, TOK_WHILE))
        return parse_while_stmt(p);
    if (match(p, TOK_SHIELD))
        return parse_shield_stmt(p);
...
static AstNode* parse_var_decl(Parser* p) {
    ...
    AstNode* node =
        ast_new_node(is_const ? NODE_CONST_DECL : NODE_VAR_DECL, tok->line, tok->column);
...
static AstNode* parse_realm_decl(Parser* p) {
    ...
    while (!check(p, TOK_RBRACE) && !is_at_end(p)) {
        AstNode* decl = parse_declaration(p);
        if (decl)
            ast_node_list_push(&node->as.realm.declarations, decl);
    }
...
static AstNode* parse_declaration(Parser* p) {
    ...
    if (match(p, TOK_MORPH))
        return parse_var_decl(p);
    if (match(p, TOK_ETERNAL))
        return parse_var_decl(p);
    if (match(p, TOK_FORGE))
        return parse_forge_decl(p);
    if (match(p, TOK_ENTITY))
        return parse_entity_decl(p);
    if (match(p, TOK_REALM))
        return parse_realm_decl(p);
    if (match(p, TOK_SUMMON))
        return parse_summon_decl(p);
    if (match(p, TOK_QUEST))
        return parse_quest_decl(p);

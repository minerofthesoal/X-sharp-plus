...
static void track_alloc(void* ptr, size_t size) {
    if (tracked_count >= MAX_TRACKED_ALLOCS)
        return;
...
static void untrack_alloc(const void* ptr) {
...
XsEntity* xs_entity_new(void) {
    ...
    e->buckets = (XsEntityEntry**)calloc((size_t)e->bucket_count, sizeof(XsEntityEntry*));
    if (!e->buckets) {
        free(e);
        return NULL;
    }
    e->count = 0;
    track_alloc(e, sizeof(XsEntity) + sizeof(XsEntityEntry*) * (size_t)e->bucket_count);
    return e;
}
...
void xs_entity_set(XsEntity* e, const char* key, XsValue v) {
    ...
    if (e->count > e->bucket_count * 3 / 4) {
        ...
        XsEntityEntry** new_buckets =
            (XsEntityEntry**)calloc((size_t)new_bucket_count, sizeof(XsEntityEntry*));
        if (!new_buckets)
            return;
...
void xs_value_print(XsValue val) {
    ...
    case VAL_ABYSS:
        ...
        break;
    case VAL_ARSENAL: {
        const XsArsenal* a = (const XsArsenal*)val.object;
        if (!a) {
            printf("[]");
            break;
        }
...
bool xs_values_equal(XsValue a, XsValue b) {
    ...
    case VAL_SPARK:
        return a.spark == b.spark;
    case VAL_SCROLL: {
        if (a.scroll == b.scroll)
            return true; /* interned match */
        if (!a.scroll || !b.scroll)
            return false;
...
bool xs_is_truthy(XsValue val) {
    ...
    case VAL_BLADE:
        return val.blade != 0;
    case VAL_SPARK:
        return val.spark != 0.0;
    case VAL_SCROLL:
        return val.scroll != NULL && val.scroll[0] != '\0';
    default:
...
XsValue xs_convert_to_blade(XsValue val) {
    ...
    case VAL_BLADE:
        return val;
    case VAL_SPARK:
        return xs_blade((int64_t)val.spark);
    case VAL_FATE:
        return xs_blade(val.fate ? 1 : 0);
    case VAL_SCROLL: {
        if (!val.scroll)
            return xs_blade(0);
        char* end;
...
XsValue xs_convert_to_spark(XsValue val) {
    ...
    case VAL_SPARK:
        return val;
    case VAL_BLADE:
        return xs_spark((double)val.blade);
    case VAL_FATE:
        return xs_spark(val.fate ? 1.0 : 0.0);
    case VAL_SCROLL: {
        if (!val.scroll)
            return xs_spark(0.0);
        char* end;

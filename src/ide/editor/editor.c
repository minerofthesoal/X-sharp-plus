...
int xs_editor_new_tab(XsEditor *ed, const char *filepath) {
    if (!ed || ed->tab_count >= XS_EDITOR_MAX_TABS) return -1;
    ...
    g_signal_connect(GTK_TEXT_BUFFER(buffer), "modified-changed",
                     G_CALLBACK(on_buffer_modified), tab);
...
bool xs_editor_open_file(XsEditor *ed, const char *filepath) {
    ...
    if (idx < 0) { free(content); return false; }
...
bool xs_editor_save_current(XsEditor *ed) {
    ...
    if (page < 0 || page >= ed->tab_count) return false;

    XsEditorTab *tab = &ed->tabs[page];
    if (!tab->file_path[0]) return false;
...
void xs_editor_close_current(XsEditor *ed) {
    if (!ed) return;
    int page = gtk_notebook_get_current_page(GTK_NOTEBOOK(ed->notebook));
    if (page >= 0) xs_editor_close_tab(ed, page);
}

void xs_editor_close_tab(XsEditor *ed, int index) {
    if (!ed || index < 0 || index >= ed->tab_count) return;
...
const char *xs_editor_get_current_path(XsEditor *ed) {
    if (!ed) return NULL;
    int page = gtk_notebook_get_current_page(GTK_NOTEBOOK(ed->notebook));
    if (page < 0 || page >= ed->tab_count) return NULL;
...
void xs_editor_undo(XsEditor *ed) {
    if (!ed) return;
    int page = gtk_notebook_get_current_page(GTK_NOTEBOOK(ed->notebook));
    if (page < 0 || page >= ed->tab_count) return;

...
static void on_new_file(GtkMenuItem *item, gpointer data)    { xs_ide_app_new_file((XsIdeApp *)data); }
static void on_open_file(GtkMenuItem *item, gpointer data)   { xs_ide_app_open_file((XsIdeApp *)data); }
static void on_save_file(GtkMenuItem *item, gpointer data)   { xs_ide_app_save_file((XsIdeApp *)data); }
static void on_save_as(GtkMenuItem *item, gpointer data)     { xs_ide_app_save_file_as((XsIdeApp *)data); }
static void on_quit(GtkMenuItem *item, gpointer data)        { xs_ide_app_quit((XsIdeApp *)data); }
static void on_build(GtkMenuItem *item, gpointer data)       { xs_ide_app_build((XsIdeApp *)data); }
static void on_run(GtkMenuItem *item, gpointer data)         { /* run action */ xs_ide_app_build((XsIdeApp *)data); }
static void on_debug(GtkMenuItem *item, gpointer data)       { xs_ide_app_debug_start((XsIdeApp *)data); }

static void on_undo(GtkMenuItem *item, gpointer data) {
    XsIdeApp *ide = (XsIdeApp *)data;
    if (ide->editor) xs_editor_undo(ide->editor);
}

static void on_redo(GtkMenuItem *item, gpointer data) {
    XsIdeApp *ide = (XsIdeApp *)data;
    if (ide->editor) xs_editor_redo(ide->editor);
}

static void on_find(GtkMenuItem *item, gpointer data) {
    XsIdeApp *ide = (XsIdeApp *)data;
    if (ide->editor) xs_editor_show_find(ide->editor);
}
...
void xs_ide_app_free(XsIdeApp *ide) {
    ...
    if (ide->editor) xs_editor_free(ide->editor);
    if (ide->project_panel) xs_project_panel_free(ide->project_panel);
    if (ide->console_panel) xs_console_panel_free(ide->console_panel);
    if (ide->debug_panel) xs_debug_panel_free(ide->debug_panel);
    if (ide->project) xs_project_free(ide->project);
    if (ide->theme) xs_theme_free(ide->theme);
    if (ide->app) g_object_unref(ide->app);
...
static void build_menu_bar(XsIdeApp *ide) {
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), create_menu_item("_New", G_CALLBACK(on_new_file), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), create_menu_item("_Open...", G_CALLBACK(on_open_file), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), create_menu_item("_Save", G_CALLBACK(on_save_file), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), create_menu_item("Save _As...", G_CALLBACK(on_save_as), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), gtk_separator_menu_item_new());
    gtk_menu_shell_append(GTK_MENU_SHELL(file_menu), create_menu_item("_Quit", G_CALLBACK(on_quit), ide));
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(edit_menu), create_menu_item("_Undo", G_CALLBACK(on_undo), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(edit_menu), create_menu_item("_Redo", G_CALLBACK(on_redo), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(edit_menu), gtk_separator_menu_item_new());
    gtk_menu_shell_append(GTK_MENU_SHELL(edit_menu), create_menu_item("_Find...", G_CALLBACK(on_find), ide));
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(view_menu), create_menu_item("Toggle _Theme", G_CALLBACK(on_theme_toggle), ide));
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(build_menu), create_menu_item("_Build Project", G_CALLBACK(on_build), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(build_menu), create_menu_item("_Run", G_CALLBACK(on_run), ide));
    gtk_menu_shell_append(GTK_MENU_SHELL(build_menu), create_menu_item("_Debug", G_CALLBACK(on_debug), ide));
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(help_menu), create_menu_item("_About", G_CALLBACK(on_about), ide));
...
static void build_status_bar(XsIdeApp *ide) {
    ...
    gtk_box_pack_start(GTK_BOX(ide->status_bar), gtk_separator_new(GTK_ORIENTATION_VERTICAL), FALSE, FALSE, 0);
    ...
    gtk_box_pack_end(GTK_BOX(ide->status_bar), gtk_separator_new(GTK_ORIENTATION_VERTICAL), FALSE, FALSE, 0);
...
void xs_ide_app_open_file(XsIdeApp *ide) {
    ...
    GtkWidget *dialog = gtk_file_chooser_dialog_new(
        "Open File", GTK_WINDOW(ide->window),
...
void xs_ide_app_save_file_as(XsIdeApp *ide) {
    if (!ide || !ide->window || !ide->editor) return;
    GtkWidget *dialog = gtk_file_chooser_dialog_new(
        "Save File As", GTK_WINDOW(ide->window),
...
void xs_ide_app_open_project(XsIdeApp *ide) {
    if (!ide || !ide->window) return;
    GtkWidget *dialog = gtk_file_chooser_dialog_new(
        "Open Project Directory", GTK_WINDOW(ide->window),

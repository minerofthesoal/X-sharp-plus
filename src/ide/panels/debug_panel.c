...
enum { VAR_COL_NAME, VAR_COL_TYPE, VAR_COL_VALUE, VAR_NUM_COLS };

/* Call stack store columns */
enum { STACK_COL_FRAME, STACK_COL_FUNCTION, STACK_COL_LOCATION, STACK_NUM_COLS };
...
XsDebugPanel* xs_debug_panel_new(void) {
    ...
    panel->var_store =
        gtk_list_store_new(VAR_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(var_scrolled), GTK_POLICY_AUTOMATIC,
    ...
    panel->stack_store =
        gtk_list_store_new(STACK_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    renderer = gtk_cell_renderer_text_new();
    col = gtk_tree_view_column_new_with_attributes("Function", renderer, "text", STACK_COL_FUNCTION,
                                                   NULL);
    gtk_tree_view_column_set_resizable(col, TRUE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(panel->stack_tree_view), col);

    renderer = gtk_cell_renderer_text_new();
    col = gtk_tree_view_column_new_with_attributes("Location", renderer, "text", STACK_COL_LOCATION,
                                                   NULL);
    gtk_tree_view_column_set_resizable(col, TRUE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(panel->stack_tree_view), col);

    GtkWidget* stack_scrolled = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(stack_scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
...
void xs_debug_panel_free(XsDebugPanel* panel) {
    if (!panel)
        return;
    if (panel->var_store)
        g_object_unref(panel->var_store);
    if (panel->stack_store)
        g_object_unref(panel->stack_store);
    free(panel);
}
...
void xs_debug_panel_update_variables(XsDebugPanel* panel, const char** names, const char** types,
    ...
    for (int i = 0; i < count; i++) {
        ...
        gtk_list_store_set(panel->var_store, &iter, VAR_COL_NAME, names[i], VAR_COL_TYPE, types[i],
                           VAR_COL_VALUE, values[i], -1);
    }
}
...
void xs_debug_panel_update_call_stack(XsDebugPanel* panel, const char** func_names,
    ...
    for (int i = 0; i < count; i++) {
        ...
        gtk_list_store_set(panel->stack_store, &iter, STACK_COL_FRAME, frame_num,
                           STACK_COL_FUNCTION, func_names[i], STACK_COL_LOCATION, locations[i], -1);
    }
}

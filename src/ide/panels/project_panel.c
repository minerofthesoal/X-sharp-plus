...
static void on_row_activated(GtkTreeView *tree_view, GtkTreePath *path,
    ...
    gboolean is_dir;
    gchar *filepath;
    gtk_tree_model_get(GTK_TREE_MODEL(panel->tree_store), &iter,
                       COL_PATH, &filepath,
                       COL_IS_DIR, &is_dir,
                       -1);
...
XsProjectPanel *xs_project_panel_new(void) {
    ...
    panel->tree_store = gtk_tree_store_new(NUM_COLS,
                                            G_TYPE_STRING,
                                            G_TYPE_STRING,
                                            G_TYPE_STRING,
                                            G_TYPE_BOOLEAN);
...
void xs_project_panel_free(XsProjectPanel *panel) {
    if (!panel) return;
    if (panel->tree_store) g_object_unref(panel->tree_store);
    free(panel);
}
...
void xs_project_panel_refresh(XsProjectPanel *panel) {
    if (!panel || !panel->root_dir[0]) return;
    ...
    gtk_tree_store_set(panel->tree_store, &root_iter,
                       COL_ICON, "folder",
                       COL_NAME, basename,
                       COL_PATH, panel->root_dir,
                       COL_IS_DIR, TRUE,
                       -1);

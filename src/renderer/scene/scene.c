...
void scene_node_add_child(SceneNode *parent, SceneNode *child) {
    ...
    if (parent->child_count >= SCENE_MAX_CHILDREN) return;
    child->parent = parent;
...
Vec3 scene_node_world_position(const SceneNode *node) {
    ...
    return vec3(node->world_matrix.m[3][0],
                node->world_matrix.m[3][1],
                node->world_matrix.m[3][2]);
}
...
Mat4 camera_get_view_matrix(const SceneNode *cam) {
    if (!cam || cam->type != NODE_CAMERA) return mat4_identity();
...
static void render_node(Scene *scene, SceneNode *node,
                         XSharpRenderer *renderer,
                         Mat4 view, Mat4 proj, Vec3 eye_pos,
    ...
    if (node->type == NODE_MESH && node->data.mesh_data.mesh) {
        ...
        PipelineTransforms xforms = pipeline_compute_transforms(
        ...
        if (mat.shader && scene->light_count > 0) {
            ...
            for (int i = 0; i < scene->light_count; i++) {
                ...
                if (l->type == LIGHT_DIRECTIONAL) {
                    ...
                    Vec3 world_dir = vec3_normalize(
                        mat4_mul_dir(ln->world_matrix, l->direction));
                    shader_set_vec3(mat.shader, "light_dir", world_dir);
                    shader_set_vec3(mat.shader, "light_color",
                                    vec3_mul(l->color, l->intensity));
                    break;
                }
            }
...
void scene_render(Scene *scene, XSharpRenderer *renderer) {
    if (!scene || !renderer || !scene->root) return;

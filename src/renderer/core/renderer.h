...
static inline Color color_from_vec4(Vec4 v) {
    return (Color){(uint8_t)(clampf(v.x, 0, 1) * 255.0f), (uint8_t)(clampf(v.y, 0, 1) * 255.0f),
                   (uint8_t)(clampf(v.z, 0, 1) * 255.0f), (uint8_t)(clampf(v.w, 0, 1) * 255.0f)};
}
...
typedef enum { BLEND_NONE, BLEND_ALPHA, BLEND_ADDITIVE, BLEND_MULTIPLY } BlendMode;

/* ------------------------------------------------------------------ */
/*  Texture filter                                                     */
/* ------------------------------------------------------------------ */

typedef enum { TEX_FILTER_NEAREST, TEX_FILTER_BILINEAR } TexFilter;

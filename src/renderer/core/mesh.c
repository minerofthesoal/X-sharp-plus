...
Mesh* mesh_create(const Vertex* verts, uint32_t vert_count, const uint32_t* indices,
                  uint32_t idx_count) {
...
#define FACE(p0, p1, p2, p3, n)                                                                    \
    ...
    /* +Z face */
    FACE(((Vec3){-h, -h, h}), ((Vec3){h, -h, h}), ((Vec3){h, h, h}), ((Vec3){-h, h, h}),
         ((Vec3){0, 0, 1}));
    /* -Z face */
    FACE(((Vec3){h, -h, -h}), ((Vec3){-h, -h, -h}), ((Vec3){-h, h, -h}), ((Vec3){h, h, -h}),
         ((Vec3){0, 0, -1}));
...
Mesh* mesh_create_cylinder(float radius, float height, int segments) {
    ...
    for (int i = 0; i < segments; i++) {
        ...
        indices[ii++] = top_base + i;
        indices[ii++] = top_base + i + 1;
    }
    ...
    for (int i = 0; i < segments; i++) {
        ...
        indices[ii++] = bot_base + i + 1;
        indices[ii++] = bot_base + i;
    }
...
static void fa_init(FloatArray* a) {
    a->data = NULL;
    a->count = 0;
    a->cap = 0;
}
...
static void ua_init(UintArray* a) {
    a->data = NULL;
    a->count = 0;
    a->cap = 0;
}
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (line[0] == 'v' && line[1] == ' ') {
            ...
            while (*p && face_count < 16) {
                ...
                if (vi < 0)
                    vi = positions.count / 3 + vi + 1;
                if (ti < 0)
                    ti = texcoords.count / 2 + ti + 1;
                if (ni < 0)
                    ni = normals.count / 3 + ni + 1;
                    ...
                    for (int k = 0; k < map.cap; k++) {
                        ...
                        if (e->v != 0 || e->vt != 0 || e->vn != 0) {
                            ...
                            for (int j = 0; j < new_cap; j++) {
                                ...
                                if (new_entries[idx2].v == 0 && new_entries[idx2].vt == 0 &&
                                    new_entries[idx2].vn == 0) {
                                    new_entries[idx2] = *e;
                                    break;
                                }
                            }
                ...
                if (existing >= 0) {
                    ...
                    if (vert_count >= vert_cap) {
                        vert_cap *= 2;
                        Vertex* tmp = realloc(out_verts, sizeof(Vertex) * vert_cap);
                        if (!tmp) {
                            free(out_verts);
                            return NULL;
                        }
                    ...
                    if (vi >= 1 && (vi - 1) * 3 + 2 < positions.count) {
                        v.position =
                            vec3(positions.data[(vi - 1) * 3], positions.data[(vi - 1) * 3 + 1],
                                 positions.data[(vi - 1) * 3 + 2]);
                    }
                    if (ti >= 1 && (ti - 1) * 2 + 1 < texcoords.count) {
                        v.uv = vec2(texcoords.data[(ti - 1) * 2], texcoords.data[(ti - 1) * 2 + 1]);
                    }
                    if (ni >= 1 && (ni - 1) * 3 + 2 < normals.count) {
                        v.normal = vec3(normals.data[(ni - 1) * 3], normals.data[(ni - 1) * 3 + 1],

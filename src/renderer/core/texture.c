...
static int format_bpp(TexFormat fmt) {
    switch (fmt) {
    case TEX_FMT_RGBA8: return 4;
    case TEX_FMT_RGB8:  return 3;
    case TEX_FMT_GRAY8: return 1;
    }
    return 4;
}
...
static Color fetch_texel(const MipLevel *mip, int x, int y) {
    ...
    return (Color){mip->pixels[idx], mip->pixels[idx+1],
                   mip->pixels[idx+2], mip->pixels[idx+3]};
}
...
XSharpTexture *texture_create(int width, int height, TexFormat fmt, const uint8_t *pixels) {
    ...
    if (!tex->mips[0].pixels) { free(tex); return NULL; }
    return tex;
}
...
void texture_update(XSharpTexture *tex, int x, int y, int w, int h,
                    TexFormat fmt, const uint8_t *pixels) {
    ...
    for (int row = 0; row < h; row++) {
        int dy = y + row;
        if (dy < 0 || dy >= mip->height) continue;
        for (int col = 0; col < w; col++) {
            int dx = x + col;
            if (dx < 0 || dx >= mip->width) continue;
...
void texture_generate_mipmaps(XSharpTexture *tex) {
    ...
    while ((w > 1 || h > 1) && tex->mip_count < TEXTURE_MAX_MIPS) {
        ...
        if (!tex->mips[level].pixels) break;
...
Color texture_sample_mip(const XSharpTexture *tex, int mip, float u, float v, TexFilter filter) {
    if (!tex || mip < 0 || mip >= tex->mip_count) return COLOR_BLACK;

    const MipLevel *lvl = &tex->mips[mip];
    if (!lvl->pixels) return COLOR_BLACK;
...
XSharpTexture *texture_load_tga(const char *path) {
    ...
    uint8_t header[18];
    if (fread(header, 1, 18, fp) != 18) { fclose(fp); return NULL; }
    ...
    if (image_type != 2 && image_type != 3) { fclose(fp); return NULL; }
    if (bpp != 24 && bpp != 32 && bpp != 8) { fclose(fp); return NULL; }

    /* Skip image ID */
    if (id_length > 0) fseek(fp, id_length, SEEK_CUR);
...
XSharpTexture *texture_load_bmp(const char *path) {
    ...
    if (fread(file_header, 1, 14, fp) != 14) { fclose(fp); return NULL; }
    if (file_header[0] != 'B' || file_header[1] != 'M') { fclose(fp); return NULL; }
    ...
    uint8_t dib[40];
    if (fread(dib, 1, 40, fp) != 40) { fclose(fp); return NULL; }
    ...
    if (compr != 0) { fclose(fp); return NULL; }
    if (bpp != 24 && bpp != 32) { fclose(fp); return NULL; }
    if (width <= 0) { fclose(fp); return NULL; }
    ...
    if (!row_buf || !rgba) { free(row_buf); free(rgba); fclose(fp); return NULL; }

    for (int y = 0; y < height; y++) {
        int dst_y = top_down ? y : (height - 1 - y);
        if (fread(row_buf, 1, row_size, fp) != (size_t)row_size) {
            free(row_buf); free(rgba); fclose(fp); return NULL;
        }
        for (int x = 0; x < width; x++) {
            int si = x * src_bpp;
...
TextureAtlas *atlas_create(int width, int height) {
    ...
    if (!atlas->texture) { free(atlas); return NULL; }

    atlas->region_capacity = 64;
    atlas->regions = malloc(sizeof(AtlasRegion) * atlas->region_capacity);
    if (!atlas->regions) { texture_destroy(atlas->texture); free(atlas); return NULL; }
...
int atlas_add(TextureAtlas *atlas, int w, int h, TexFormat fmt, const uint8_t *pixels) {
    if (!atlas || !pixels || w <= 0 || h <= 0) return -1;
    ...
    if (atlas->cursor_y + h > ah) return -1; /* atlas full */
    ...
    atlas->cursor_x += w;
    if (h > atlas->row_height) atlas->row_height = h;

    return idx;
}

const AtlasRegion *atlas_get_region(const TextureAtlas *atlas, int index) {
    if (!atlas || index < 0 || index >= atlas->region_count) return NULL;

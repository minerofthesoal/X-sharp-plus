...
Mat4 mat4_inverse(Mat4 m) {
    ...
    inv[0]  =  S(1,1)*(S(2,2)*S(3,3)-S(3,2)*S(2,3)) - S(2,1)*(S(1,2)*S(3,3)-S(3,2)*S(1,3)) + S(3,1)*(S(1,2)*S(2,3)-S(2,2)*S(1,3));
    inv[1]  = -S(0,1)*(S(2,2)*S(3,3)-S(3,2)*S(2,3)) + S(2,1)*(S(0,2)*S(3,3)-S(3,2)*S(0,3)) - S(3,1)*(S(0,2)*S(2,3)-S(2,2)*S(0,3));
    inv[2]  =  S(0,1)*(S(1,2)*S(3,3)-S(3,2)*S(1,3)) - S(1,1)*(S(0,2)*S(3,3)-S(3,2)*S(0,3)) + S(3,1)*(S(0,2)*S(1,3)-S(1,2)*S(0,3));
    inv[3]  = -S(0,1)*(S(1,2)*S(2,3)-S(2,2)*S(1,3)) + S(1,1)*(S(0,2)*S(2,3)-S(2,2)*S(0,3)) - S(2,1)*(S(0,2)*S(1,3)-S(1,2)*S(0,3));

    det = S(0,0)*inv[0] + S(1,0)*inv[1] + S(2,0)*inv[2] + S(3,0)*inv[3];
    if (fabsf(det) < XSHARP_EPSILON) return mat4_identity();
    det = 1.0f / det;

    inv[4]  = -S(1,0)*(S(2,2)*S(3,3)-S(3,2)*S(2,3)) + S(2,0)*(S(1,2)*S(3,3)-S(3,2)*S(1,3)) - S(3,0)*(S(1,2)*S(2,3)-S(2,2)*S(1,3));
    inv[5]  =  S(0,0)*(S(2,2)*S(3,3)-S(3,2)*S(2,3)) - S(2,0)*(S(0,2)*S(3,3)-S(3,2)*S(0,3)) + S(3,0)*(S(0,2)*S(2,3)-S(2,2)*S(0,3));
    inv[6]  = -S(0,0)*(S(1,2)*S(3,3)-S(3,2)*S(1,3)) + S(1,0)*(S(0,2)*S(3,3)-S(3,2)*S(0,3)) - S(3,0)*(S(0,2)*S(1,3)-S(1,2)*S(0,3));
    inv[7]  =  S(0,0)*(S(1,2)*S(2,3)-S(2,2)*S(1,3)) - S(1,0)*(S(0,2)*S(2,3)-S(2,2)*S(0,3)) + S(2,0)*(S(0,2)*S(1,3)-S(1,2)*S(0,3));

    inv[8]  =  S(1,0)*(S(2,1)*S(3,3)-S(3,1)*S(2,3)) - S(2,0)*(S(1,1)*S(3,3)-S(3,1)*S(1,3)) + S(3,0)*(S(1,1)*S(2,3)-S(2,1)*S(1,3));
    inv[9]  = -S(0,0)*(S(2,1)*S(3,3)-S(3,1)*S(2,3)) + S(2,0)*(S(0,1)*S(3,3)-S(3,1)*S(0,3)) - S(3,0)*(S(0,1)*S(2,3)-S(2,1)*S(0,3));
    inv[10] =  S(0,0)*(S(1,1)*S(3,3)-S(3,1)*S(1,3)) - S(1,0)*(S(0,1)*S(3,3)-S(3,1)*S(0,3)) + S(3,0)*(S(0,1)*S(1,3)-S(1,1)*S(0,3));
    inv[11] = -S(0,0)*(S(1,1)*S(2,3)-S(2,1)*S(1,3)) + S(1,0)*(S(0,1)*S(2,3)-S(2,1)*S(0,3)) - S(2,0)*(S(0,1)*S(1,3)-S(1,1)*S(0,3));

    inv[12] = -S(1,0)*(S(2,1)*S(3,2)-S(3,1)*S(2,2)) + S(2,0)*(S(1,1)*S(3,2)-S(3,1)*S(1,2)) - S(3,0)*(S(1,1)*S(2,2)-S(2,1)*S(1,2));
    inv[13] =  S(0,0)*(S(2,1)*S(3,2)-S(3,1)*S(2,2)) - S(2,0)*(S(0,1)*S(3,2)-S(3,1)*S(0,2)) + S(3,0)*(S(0,1)*S(2,2)-S(2,1)*S(0,2));
    inv[14] = -S(0,0)*(S(1,1)*S(3,2)-S(3,1)*S(1,2)) + S(1,0)*(S(0,1)*S(3,2)-S(3,1)*S(0,2)) - S(3,0)*(S(0,1)*S(1,2)-S(1,1)*S(0,2));
    inv[15] =  S(0,0)*(S(1,1)*S(2,2)-S(2,1)*S(1,2)) - S(1,0)*(S(0,1)*S(2,2)-S(2,1)*S(0,2)) + S(2,0)*(S(0,1)*S(1,2)-S(1,1)*S(0,2));
    #undef S
...
Mat4 mat4_from_quat(Quat q) { return quat_to_mat4(q); }

/* ================================================================== */
/*  Quaternion                                                         */
/* ================================================================== */

Quat quat_identity(void) { return (Quat){0,0,0,1}; }
...
Quat quat_normalize(Quat q) {
    ...
    if (len < XSHARP_EPSILON) return quat_identity();
    float inv = 1.0f / len;
...
Quat quat_slerp(Quat a, Quat b, float t) {
    ...
    if (dot < 0.0f) { b = (Quat){-b.x,-b.y,-b.z,-b.w}; dot = -dot; }
    if (dot > 0.9995f) {
...
bool ray_plane_intersect(Ray ray, Plane plane, float *t_out) {
    ...
    if (fabsf(denom) < XSHARP_EPSILON) return false;
...
bool ray_triangle_intersect(Ray ray, Vec3 v0, Vec3 v1, Vec3 v2,
    ...
    if (fabsf(a) < XSHARP_EPSILON) return false;
    float f = 1.0f / a;
    ...
    if (t < XSHARP_EPSILON) return false;
    if (t_out) *t_out = t;
    if (u_out) *u_out = u;
    if (v_out) *v_out = v;
    return true;
...
bool ray_aabb_intersect(Ray ray, AABB box, float *t_out) {
    ...
    if (t_out) *t_out = tmin >= 0 ? tmin : tmax;
    return true;
}

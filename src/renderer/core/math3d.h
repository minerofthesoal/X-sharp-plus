...
static inline Vec2 vec2_normalize(Vec2 v) {
    ...
    return l > XSHARP_EPSILON ? vec2_div(v, l) : v;
}
...
static inline Vec3 vec3_normalize(Vec3 v) {
    ...
    return l > XSHARP_EPSILON ? vec3_div(v, l) : v;
}
static inline Vec3 vec3_lerp(Vec3 a, Vec3 b, float t) {
    return vec3_add(vec3_mul(a, 1 - t), vec3_mul(b, t));
}
...
static inline Vec4 vec4_lerp(Vec4 a, Vec4 b, float t) {
    return vec4_add(vec4_mul(a, 1 - t), vec4_mul(b, t));
}

 ...
 */
static int clip_against_plane(const ClipVertex* in, int in_count, ClipVertex* out, int axis,
                              float sign) {
    if (in_count == 0)
        return 0;
    int out_count = 0;
    ...
    for (int i = 0; i < in_count; i++) {
        ...
        if (prev_inside != cur_inside) {
            ...
            float t = dp / (dp - dc);
            if (out_count < 12)
                out[out_count++] = clip_lerp(prev, cur, t);
        }
        if (cur_inside) {
            if (out_count < 12)
                out[out_count++] = *cur;
...
static int clip_triangle(const ClipVertex tri[3], ClipVertex* out) {
    ...
    /* +x: x <= w  =>  +1*x - w <= 0 */
    count = clip_against_plane(buf_a, count, buf_b, 0, 1.0f);
    if (count < 3)
        return 0;
    /* -x: -x <= w => -1*x - w <= 0 */
    count = clip_against_plane(buf_b, count, buf_a, 0, -1.0f);
    if (count < 3)
        return 0;
    /* +y */
    count = clip_against_plane(buf_a, count, buf_b, 1, 1.0f);
    if (count < 3)
        return 0;
    /* -y */
    count = clip_against_plane(buf_b, count, buf_a, 1, -1.0f);
    if (count < 3)
        return 0;
    /* +z (far) */
    count = clip_against_plane(buf_a, count, buf_b, 2, 1.0f);
    if (count < 3)
        return 0;
    ...
    count = clip_against_plane(buf_b, count, buf_a, 2, -1.0f);
    if (count < 3)
        return 0;
...
static float shadow_test(const PipelineMaterial* mat, Vec3 world_pos) {
    if (!mat->shadow_enabled || !mat->shadow_map)
        return 1.0f;
 ...
 */
static void rasterize_with_shader(XSharpRenderer* r, const ClipVertex* cv, int vert_count,
                                  const PipelineMaterial* mat, const Rect* vp) {
    ...
    for (int t = 2; t < vert_count; t++) {
        ...
        for (int py = miny; py <= maxy; py++) {
            for (int px = minx; px <= maxx; px++) {
                ...
                /* Depth */
                float depth =
                    l0 * tri[0]->position.z + l1 * tri[1]->position.z + l2 * tri[2]->position.z;
                if (depth < 0.0f || depth > 1.0f)
                    continue;

                /* Depth test */
                int fi = py * fb->width + px;
                if (st->depth_test && depth >= fb->depth[fi])
                    continue;
                ...
                if (shader && shader->fragment_fn) {
                    ...
                    frag_color =
                        vec4(frag_vary.v[8], frag_vary.v[9], frag_vary.v[10], frag_vary.v[11]);
                }
...
void pipeline_render_triangle(XSharpRenderer* renderer, const Vertex* v0, const Vertex* v1,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (shader && shader->vertex_fn) {
            ...
            clip_tri[i].vary.v[0] = wp.x;
            clip_tri[i].vary.v[1] = wp.y;
            clip_tri[i].vary.v[2] = wp.z;
            clip_tri[i].vary.v[3] = wn.x;
            clip_tri[i].vary.v[4] = wn.y;
            clip_tri[i].vary.v[5] = wn.z;
            clip_tri[i].vary.v[6] = vin.uv.x;
            clip_tri[i].vary.v[7] = vin.uv.y;
            clip_tri[i].vary.v[8] = vin.color.x;
            clip_tri[i].vary.v[9] = vin.color.y;
            clip_tri[i].vary.v[10] = vin.color.z;
            clip_tri[i].vary.v[11] = vin.color.w;
        }
...
void pipeline_render_mesh(XSharpRenderer* renderer, const Mesh* mesh,
    ...
    for (uint32_t i = 0; i + 2 < mesh->index_count; i += 3) {
        ...
        pipeline_render_triangle(renderer, &mesh->vertices[i0], &mesh->vertices[i1],
                                 &mesh->vertices[i2], xforms, material);
    }
}

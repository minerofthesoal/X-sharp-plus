...
ShaderProgram *shader_create(const char *name, VertexShaderFn vs, FragmentShaderFn fs, int varying_count) {
    ShaderProgram *prog = calloc(1, sizeof(ShaderProgram));
    if (!prog) return NULL;
    if (name) strncpy(prog->name, name, sizeof(prog->name) - 1);
    prog->vertex_fn     = vs;
    prog->fragment_fn   = fs;
...
static ShaderUniform *find_or_create_uniform(ShaderProgram *prog, const char *name) {
    ...
    if (prog->uniform_count >= SHADER_MAX_UNIFORMS) return NULL;
...
static VertexOutput common_vertex(const VertexInput *in,
                                   const ShaderUniform *u, int uc) {
    VertexOutput out;
...
static Vec4 unlit_fragment(const FragmentInput *in,
                            const ShaderUniform *u, int uc) {
    Vec4 color = {in->varyings.v[8], in->varyings.v[9],
                  in->varyings.v[10], in->varyings.v[11]};
    ...
    if (tex) {
        ...
        color.x *= tv.x; color.y *= tv.y; color.z *= tv.z; color.w *= tv.w;
    }
    return color;
}
...
static Vec4 diffuse_fragment(const FragmentInput *in,
                              const ShaderUniform *u, int uc) {
    ...
    Vec4 base_color = {in->varyings.v[8], in->varyings.v[9],
                       in->varyings.v[10], in->varyings.v[11]};
    ...
    if (tex) {
        ...
        Vec4 tv = color_to_vec4(tc);
        base_color.x *= tv.x; base_color.y *= tv.y; base_color.z *= tv.z; base_color.w *= tv.w;
    }

    Vec3 diffuse = vec3_add(ambient, vec3_mul(light_col, ndl));
    return vec4(
        clampf(base_color.x * diffuse.x, 0, 1),
        clampf(base_color.y * diffuse.y, 0, 1),
        clampf(base_color.z * diffuse.z, 0, 1),
        base_color.w
    );
}
...
static Vec4 phong_fragment(const FragmentInput *in,
                            const ShaderUniform *u, int uc) {
    ...
    Vec3 color_rgb = vec3_add(
        vec3_add(ambient, vec3_mul(light_col, ndl)),
        vec3_mul(spec_col, spec)
    );

    return vec4(
        clampf(base_color.x * color_rgb.x, 0, 1),
        clampf(base_color.y * color_rgb.y, 0, 1),
        clampf(base_color.z * color_rgb.z, 0, 1),
        base_color.w
    );
}
...
static Vec4 toon_fragment(const FragmentInput *in,
                           const ShaderUniform *u, int uc) {
    Vec3 normal    = vec3_normalize(vec3(in->varyings.v[3], in->varyings.v[4], in->varyings.v[5]));
    Vec3 light_dir = vec3_normalize(get_vec3(u, uc, "light_dir", vec3(0, -1, 0)));
    Vec3 light_col = get_vec3(u, uc, "light_color", vec3(1, 1, 1));
    ...
    return vec4(
        clampf(base_color.x * lit.x, 0, 1),
        clampf(base_color.y * lit.y, 0, 1),
        clampf(base_color.z * lit.z, 0, 1),
        base_color.w
    );
}
...
static Vec4 wireframe_fragment(const FragmentInput *in,
                                const ShaderUniform *u, int uc) {
    (void)u; (void)uc;
    return vec4(in->varyings.v[8], in->varyings.v[9],
                in->varyings.v[10], in->varyings.v[11]);
}

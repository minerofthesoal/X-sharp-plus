...
GpuContext* gpu_context_create(void) {
    ...
    if (err == CL_SUCCESS && num_platforms > 0) {
        ...
        if (err == CL_SUCCESS) {
            ...
            if (err != CL_SUCCESS)
                err =
                    clGetDeviceIDs(ctx->cl_platform, CL_DEVICE_TYPE_CPU, 1, &ctx->cl_device, NULL);
            ...
            if (err == CL_SUCCESS) {
                ...
                if (err == CL_SUCCESS) {
                    ...
                    if (err == CL_SUCCESS) {
                        ...
                        clGetDeviceInfo(ctx->cl_device, CL_DEVICE_NAME, sizeof(ctx->device_name),
...
void gpu_context_destroy(GpuContext* ctx) {
    ...
    if (ctx->backend == GPU_BACKEND_OPENCL) {
        if (ctx->cl_queue)
            clReleaseCommandQueue(ctx->cl_queue);
        if (ctx->cl_ctx)
            clReleaseContext(ctx->cl_ctx);
    }
#endif
...
GpuError gpu_buffer_upload(GpuContext* ctx, GpuBuffer* buf, const void* data, size_t offset,
                           size_t size) {
    if (!ctx || !buf || !data)
...
#ifdef XSHARP_GPU_OPENCL
    if (ctx->backend == GPU_BACKEND_OPENCL) {
        cl_int err = clEnqueueWriteBuffer(ctx->cl_queue, buf->cl_buffer, CL_TRUE, offset, size,
                                          data, 0, NULL, NULL);
...
GpuError gpu_buffer_download(GpuContext* ctx, GpuBuffer* buf, void* data, size_t offset,
                             size_t size) {
    if (!ctx || !buf || !data)
        return GPU_ERROR_INVALID_ARG;
    if (offset + size > buf->size)
...
#ifdef XSHARP_GPU_OPENCL
    if (ctx->backend == GPU_BACKEND_OPENCL) {
        cl_int err = clEnqueueReadBuffer(ctx->cl_queue, buf->cl_buffer, CL_TRUE, offset, size, data,
                                         0, NULL, NULL);
        return (err == CL_SUCCESS) ? GPU_OK : GPU_ERROR_ALLOC;
    }
#endif
...
GpuKernel* gpu_kernel_create(GpuContext* ctx, const char* source, const char* kernel_name,
                             CpuKernelFn cpu_fn) {
    if (!ctx)
        return NULL;
    ...
    if (ctx->backend == GPU_BACKEND_OPENCL && source && kernel_name) {
        ...
        if (err != CL_SUCCESS) {
            ...
            if (log_size > 1) {
                ...
                if (log) {
                    clGetProgramBuildInfo(prog, ctx->cl_device, CL_PROGRAM_BUILD_LOG, log_size, log,
                                          NULL);
...
GpuError gpu_kernel_set_buffer(GpuContext* ctx, GpuKernel* kernel, int arg_index, GpuBuffer* buf) {
    if (!ctx || !kernel || !buf || arg_index < 0 || arg_index >= GPU_MAX_KERNEL_ARGS)
        return GPU_ERROR_INVALID_ARG;

#ifdef XSHARP_GPU_OPENCL
    if (ctx->backend == GPU_BACKEND_OPENCL) {
        cl_int err = clSetKernelArg(kernel->cl_kernel, arg_index, sizeof(cl_mem), &buf->cl_buffer);
...
GpuError gpu_kernel_set_value(GpuContext* ctx, GpuKernel* kernel, int arg_index, const void* value,
                              size_t size) {
...
GpuError gpu_dispatch(GpuContext* ctx, GpuKernel* kernel, uint32_t global_size,
                      uint32_t local_size) {
    if (!ctx || !kernel)
        return GPU_ERROR_INVALID_ARG;
    ...
    if (ctx->backend == GPU_BACKEND_OPENCL) {
        ...
        cl_int err = clEnqueueNDRangeKernel(ctx->cl_queue, kernel->cl_kernel, 1, NULL, &gs,
                                            ls > 0 ? &ls : NULL, 0, NULL, NULL);
        return (err == CL_SUCCESS) ? GPU_OK : GPU_ERROR_KERNEL;
    }
#endif
...
GpuError gpu_dispatch_2d(GpuContext* ctx, GpuKernel* kernel, uint32_t gx, uint32_t gy, uint32_t lx,
                         uint32_t ly) {
    if (!ctx || !kernel)
        return GPU_ERROR_INVALID_ARG;
    ...
    if (ctx->backend == GPU_BACKEND_OPENCL) {
        ...
        cl_int err = clEnqueueNDRangeKernel(ctx->cl_queue, kernel->cl_kernel, 2, NULL, gs,
                                            use_local ? ls : NULL, 0, NULL, NULL);
        return (err == CL_SUCCESS) ? GPU_OK : GPU_ERROR_KERNEL;
    }
#endif

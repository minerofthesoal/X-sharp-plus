...
static Lz77Match find_match(LzmaEncoder* enc, const uint8_t* data, size_t data_size, size_t pos) {
    Lz77Match best = {0, 0};
    if (pos + 2 >= data_size)
        return best;
    ...
    if (max_len > 273)
        max_len = 273; /* LZMA max match length */
...
static void encode_distance(LzmaEncoder* enc, uint32_t dist, uint32_t len) {
    ...
    if (slot >= 4) {
        ...
        if (slot < 14) {
            ...
            rc_encode_tree_reverse(&enc->rc, enc->dist_special + base - slot - 1, footer_bits,
                                   remainder);
        } else {
...
static uint32_t decode_distance(LzmaDecoder* dec, uint32_t len) {
    ...
    if (slot < 14) {
        remainder =
            rc_decode_tree_reverse(&dec->rd, dec->dist_special + base - slot - 1, footer_bits);
    } else {
...
bool lzma2_compress(const uint8_t* data, size_t data_size, uint8_t** out, size_t* out_size,
    ...
    if (level < LZMA2_MIN_LEVEL)
        level = LZMA2_MIN_LEVEL;
    if (level > LZMA2_MAX_LEVEL)
        level = LZMA2_MAX_LEVEL;
    if (dict_size == 0)
        dict_size = lzma2_dict_size_for_level(level);
    ...
    while (pos < data_size) {
        ...
        size_t comp_size = 0;
        bool compressed = lzma_encode_block(&enc, data + pos, chunk_size, &comp_data, &comp_size);
...
bool lzma2_decompress(const uint8_t* comp, size_t comp_size, uint8_t* out, size_t out_size,
    ...
    while (cpos < comp_size) {
        ...
        if (chunk_type == LZMA2_CHUNK_END) {
            ...
            cpos += 2;
            if (cpos + size > comp_size)
                return false;
            if (opos + size > out_size)
                return false;
            ...
            uint32_t uncomp_size =
                ((uint32_t)comp[cpos] << 16) | ((uint32_t)comp[cpos + 1] << 8) | comp[cpos + 2];
            cpos += 3;
            uint16_t comp_chunk_size = ((uint16_t)comp[cpos] << 8) | comp[cpos + 1];
            cpos += 2;
            if (cpos + comp_chunk_size > comp_size)
                return false;
            if (opos + uncomp_size > out_size)
                return false;

            LzmaDecoder dec;
            if (!lzma_decoder_init(&dec, dict_size))
                return false;
            bool ok =
                lzma_decode_block(&dec, comp + cpos, comp_chunk_size, out + opos, uncomp_size);

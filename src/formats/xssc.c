...
bool xssc_set_metadata(XsscArchive *archive, const char *key, const char *value) {
    if (!archive || !key || !value) return false;
    if (strlen(key) > XSSC_MAX_KEY_LEN) return false;
    if (strlen(value) > XSSC_MAX_VAL_LEN) return false;
    ...
    /* New key */
    if (archive->metadata_count >= XSSC_MAX_METADATA) return false;
    if (archive->metadata_count >= archive->metadata_capacity) {
        int new_cap = archive->metadata_capacity * 2;
        XsscMetadata *tmp = (XsscMetadata *)realloc(
            archive->metadata, sizeof(XsscMetadata) * new_cap);
        if (!tmp) return false;
...
static int add_entry(XsscArchive *archive, const char *archive_path,
                     const uint8_t *data, size_t size, XsscEntryType type) {
    if (!archive || !archive_path || !data) return -1;
    if (strlen(archive_path) > XSSC_MAX_PATH_LEN) return -1;
    if (archive->entry_count >= XSSC_MAX_ENTRIES) return -1;

    if (archive->entry_count >= archive->entry_capacity) {
        int new_cap = archive->entry_capacity * 2;
        XsscEntry *tmp = (XsscEntry *)realloc(
            archive->entries, sizeof(XsscEntry) * new_cap);
        if (!tmp) return -1;
        archive->entries = tmp;
...
int xssc_add_file(XsscArchive *archive, const char *disk_path,
                  const char *archive_path, XsscEntryType type) {
    if (!archive || !disk_path || !archive_path) return -1;
...
int xssc_add_data(XsscArchive *archive, const char *archive_path,
                  const uint8_t *data, size_t size, XsscEntryType type) {
...
static bool wb_ensure(WriteBuf *wb, size_t need) {
    if (wb->size + need <= wb->cap) return true;
    size_t new_cap = wb->cap;
    while (wb->size + need > new_cap) new_cap *= 2;
...
static bool wb_write(WriteBuf *wb, const void *data, size_t len) {
    if (!wb_ensure(wb, len)) return false;
    memcpy(wb->buf + wb->size, data, len);
    wb->size += len;
    return true;
}

static bool wb_write_u8(WriteBuf *wb, uint8_t v)   { return wb_write(wb, &v, 1); }
static bool wb_write_u16(WriteBuf *wb, uint16_t v)  { return wb_write(wb, &v, 2); }
static bool wb_write_u32(WriteBuf *wb, uint32_t v)  { return wb_write(wb, &v, 4); }
static bool wb_write_u64(WriteBuf *wb, uint64_t v)  { return wb_write(wb, &v, 8); }

/* ===== Serialization ===== */

bool xssc_serialize(XsscArchive *archive, uint8_t **out_buf, size_t *out_size) {
    if (!archive || !out_buf || !out_size) return false;

    WriteBuf wb;
    if (!wb_init(&wb, 4096)) return false;

    /* ---- Header ---- */
    /* Magic */
    if (!wb_write(&wb, XSSC_MAGIC, XSSC_MAGIC_SIZE)) goto fail;
    /* Version */
    if (!wb_write_u16(&wb, archive->version)) goto fail;
    /* Flags */
    if (!wb_write_u16(&wb, archive->flags)) goto fail;
    /* Header size placeholder - we'll patch it */
    size_t header_size_off = wb.size;
    if (!wb_write_u32(&wb, 0)) goto fail;
    /* Entry count */
    if (!wb_write_u32(&wb, (uint32_t)archive->entry_count)) goto fail;

    /* ---- Metadata section ---- */
    for (int i = 0; i < archive->metadata_count; i++) {
        XsscMetadata *m = &archive->metadata[i];
        uint16_t klen = (uint16_t)strlen(m->key);
        if (!wb_write_u16(&wb, klen)) goto fail;
        if (!wb_write(&wb, m->key, klen)) goto fail;
        uint32_t vlen = m->value_len;
        if (!wb_write_u32(&wb, vlen)) goto fail;
        if (!wb_write(&wb, m->value, vlen)) goto fail;
    }
    /* Terminator: key_len=0 */
    if (!wb_write_u16(&wb, 0)) goto fail;
    ...
    for (int i = 0; i < archive->entry_count; i++) {
        ...
        if (!wb_write_u16(&wb, plen)) goto fail;
        if (!wb_write(&wb, e->path, plen)) goto fail;
        if (!wb_write_u64(&wb, e->content_offset)) goto fail;
        if (!wb_write_u64(&wb, e->content_size)) goto fail;
        if (!wb_write_u32(&wb, e->checksum)) goto fail;
        if (!wb_write_u8(&wb, (uint8_t)e->type)) goto fail;
    }
    ...
    for (int i = 0; i < archive->entry_count; i++) {
        ...
        if (e->data && e->content_size > 0) {
            if (!wb_write(&wb, e->data, (size_t)e->content_size)) goto fail;
        }
    }
    ...
    if (!wb_write_u32(&wb, total_crc)) goto fail;
    /* Reverse magic */
    if (!wb_write(&wb, XSSC_FOOTER_MAGIC, XSSC_MAGIC_SIZE)) goto fail;
...
bool xssc_write(XsscArchive *archive, const char *path) {
    ...
    if (!xssc_serialize(archive, &buf, &size)) return false;
...
static bool rb_read_u8(ReadBuf *rb, uint8_t *v)   { return rb_read(rb, v, 1); }
static bool rb_read_u16(ReadBuf *rb, uint16_t *v)  { return rb_read(rb, v, 2); }
static bool rb_read_u32(ReadBuf *rb, uint32_t *v)  { return rb_read(rb, v, 4); }
static bool rb_read_u64(ReadBuf *rb, uint64_t *v)  { return rb_read(rb, v, 8); }
...
XsscArchive *xssc_read_buffer(const uint8_t *buf, size_t size) {
    ...
    if (!rb_read(&rb, magic, XSSC_MAGIC_SIZE)) return NULL;
    if (memcmp(magic, XSSC_MAGIC, XSSC_MAGIC_SIZE) != 0) return NULL;

    /* Version */
    uint16_t version;
    if (!rb_read_u16(&rb, &version)) return NULL;

    /* Flags */
    uint16_t flags;
    if (!rb_read_u16(&rb, &flags)) return NULL;

    /* Header size */
    uint32_t header_size;
    if (!rb_read_u32(&rb, &header_size)) return NULL;

    /* Entry count */
    uint32_t entry_count;
    if (!rb_read_u32(&rb, &entry_count)) return NULL;
    if (entry_count > XSSC_MAX_ENTRIES) return NULL;
    ...
    for (;;) {
        uint16_t klen;
        if (!rb_read_u16(&rb, &klen)) goto fail;
        if (klen == 0) break;  /* terminator */
        if (klen > XSSC_MAX_KEY_LEN) goto fail;

        char *key = (char *)malloc(klen + 1);
        if (!key) goto fail;
        if (!rb_read(&rb, key, klen)) { free(key); goto fail; }
        key[klen] = '\0';

        uint32_t vlen;
        if (!rb_read_u32(&rb, &vlen)) { free(key); goto fail; }
        if (vlen > XSSC_MAX_VAL_LEN) { free(key); goto fail; }

        char *val = (char *)malloc(vlen + 1);
        if (!val) { free(key); goto fail; }
        if (!rb_read(&rb, val, vlen)) { free(key); free(val); goto fail; }
        val[vlen] = '\0';
    ...
    for (uint32_t i = 0; i < entry_count; i++) {
        uint16_t plen;
        if (!rb_read_u16(&rb, &plen)) goto fail;
        if (plen > XSSC_MAX_PATH_LEN) goto fail;

        char *path = (char *)malloc(plen + 1);
        if (!path) goto fail;
        if (!rb_read(&rb, path, plen)) { free(path); goto fail; }
        path[plen] = '\0';

        uint64_t c_offset, c_size;
        uint32_t checksum;
        uint8_t etype;
        if (!rb_read_u64(&rb, &c_offset)) { free(path); goto fail; }
        if (!rb_read_u64(&rb, &c_size)) { free(path); goto fail; }
        if (!rb_read_u32(&rb, &checksum)) { free(path); goto fail; }
        if (!rb_read_u8(&rb, &etype)) { free(path); goto fail; }

        /* Grow entries if needed */
        if (ar->entry_count >= ar->entry_capacity) {
            int new_cap = ar->entry_capacity * 2;
            XsscEntry *tmp = (XsscEntry *)realloc(
                ar->entries, sizeof(XsscEntry) * new_cap);
            if (!tmp) { free(path); goto fail; }
            ar->entries = tmp;
    ...
    for (int i = 0; i < ar->entry_count; i++) {
        ...
        if (e->content_size > 0) {
            ...
            if (abs_off + e->content_size > data_len) goto fail;
...
XsscArchive *xssc_read(const char *path) {
    ...
    if ((long)rd != fsize) { free(buf); return NULL; }
...
bool xssc_extract(const XsscArchive *archive, int entry_index,
                  const char *output_dir) {
    if (!archive || !output_dir) return false;
    if (entry_index < 0 || entry_index >= archive->entry_count) return false;

    const XsscEntry *e = &archive->entries[entry_index];
    if (!e->data && e->content_size > 0) return false;
...
static const char *entry_type_str(XsscEntryType t) {
    switch (t) {
        case XSSC_ENTRY_SOURCE:   return "source";
        case XSSC_ENTRY_ASSET:    return "asset";
        case XSSC_ENTRY_CONFIG:   return "config";
        case XSSC_ENTRY_BYTECODE: return "bytecode";
...
void xssc_list(const XsscArchive *archive) {
    ...
    for (int i = 0; i < archive->entry_count; i++) {
        ...
        printf("%-8s %10llu 0x%08X %8llu  %s\n",
               ...
               (unsigned long long)e->content_size,
               e->checksum,
               (unsigned long long)e->content_offset,
               e->path);
    }
}
...
bool xssc_validate(const XsscArchive *archive) {
    ...
    for (int i = 0; i < archive->entry_count; i++) {
        ...
        if (e->data && e->content_size > 0) {
            ...
            if (crc != e->checksum) return false;
        }
    }
    return true;
}
...
bool xssc_validate_file(const char *path) {
    ...
    size_t rd = fread(buf, 1, (size_t)fsize, f);
    fclose(f);
    if ((long)rd != fsize) { free(buf); return false; }

...
XscscArchive *xscsc_compress(XsscArchive *xssc, int compression_level) {
    ...
    uint8_t *raw = NULL;
    size_t raw_size = 0;
    if (!xssc_serialize(xssc, &raw, &raw_size)) return NULL;

    /* Determine dictionary size */
    if (compression_level < LZMA2_MIN_LEVEL) compression_level = LZMA2_DEFAULT_LEVEL;
    if (compression_level > LZMA2_MAX_LEVEL) compression_level = LZMA2_MAX_LEVEL;
...
XsscArchive *xscsc_decompress(const XscscArchive *xscsc) {
    if (!xscsc || !xscsc->compressed_data) return NULL;
    if (xscsc->compression_method != XSCSC_COMPRESS_LZMA2) return NULL;

    /* Verify checksum */
    uint32_t crc = xssc_crc32(xscsc->compressed_data, (size_t)xscsc->compressed_size);
    if (crc != xscsc->data_checksum) return NULL;

    /* Decompress */
    uint8_t *raw = (uint8_t *)malloc((size_t)xscsc->uncompressed_size);
    if (!raw) return NULL;

    if (!lzma2_decompress(xscsc->compressed_data, (size_t)xscsc->compressed_size,
                          raw, (size_t)xscsc->uncompressed_size,
                          xscsc->dictionary_size)) {
        free(raw);
        return NULL;
    }
...
bool xscsc_write(const XscscArchive *ar, const char *path) {
    if (!ar || !path || !ar->compressed_data) return false;
...
XscscArchive *xscsc_read(const char *path) {
    ...
    XscscArchive *ar = xscsc_create();
    if (!ar) { fclose(f); return NULL; }

    if (fread(&ar->version, 2, 1, f) != 1) goto fail;
    if (fread(&ar->compression_method, 1, 1, f) != 1) goto fail;
    if (fread(&ar->uncompressed_size, 8, 1, f) != 1) goto fail;
    if (fread(&ar->compressed_size, 8, 1, f) != 1) goto fail;
    if (fread(&ar->dictionary_size, 4, 1, f) != 1) goto fail;

    /* Read compressed data */
    ar->compressed_data = (uint8_t *)malloc((size_t)ar->compressed_size);
    if (!ar->compressed_data) goto fail;
    if (fread(ar->compressed_data, 1, (size_t)ar->compressed_size, f) !=
        (size_t)ar->compressed_size) goto fail;

    /* Read footer */
    if (fread(&ar->data_checksum, 4, 1, f) != 1) goto fail;

    char footer[XSCSC_MAGIC_SIZE];
    if (fread(footer, 1, XSCSC_MAGIC_SIZE, f) != XSCSC_MAGIC_SIZE ||
        memcmp(footer, XSCSC_FOOTER_MAGIC, XSCSC_MAGIC_SIZE) != 0) goto fail;
...
static bool add_dir_recursive(XsscArchive *xssc, const char *dir_path,
                              const char *prefix) {
    ...
    while ((ent = readdir(d)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            if (ext) {
                if (strcmp(ext, ".xs") == 0) type = XSSC_ENTRY_SOURCE;
                else if (strcmp(ext, ".xsb") == 0) type = XSSC_ENTRY_BYTECODE;
                else if (strcmp(ext, ".cfg") == 0 || strcmp(ext, ".json") == 0 ||
                         strcmp(ext, ".toml") == 0) type = XSSC_ENTRY_CONFIG;
            }
...
bool xscsc_create_from_dir(const char *dir_path, const char *output_path,
                           int compression_level) {
    if (!dir_path || !output_path) return false;

    XsscArchive *xssc = xssc_create();
    if (!xssc) return false;

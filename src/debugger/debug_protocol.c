...
static void dap_response(int seq, int request_seq, const char *command,
...
static int dap_read_msg(char *buf, int buf_size) {
    ...
    if (content_length <= 0 || content_length >= buf_size) return -1;
...
void xs_dap_run(XsDebugger *dbg) {
    ...
    while (running) {
        ...
        if (strcmp(command, "initialize") == 0) {
            ...
            dap_response(seq++, request_seq, "setBreakpoints", true,
                "{\"breakpoints\":[]}");
        }
            ...
            dap_response(seq++, request_seq, "continue", true,
            ...
            for (int i = 0; i < count; i++) {
                snprintf(frame_buf, sizeof(frame_buf),
                    ...
                    "\"line\":%d,\"column\":%d}",
                    i > 0 ? "," : "", frames[i].id, frames[i].func_name,
                    frames[i].source_file, frames[i].line, frames[i].column);
            ...
            int count = 0;
            if (ref == 1) count = xs_debugger_get_locals(dbg, 0, vars, 32);
            else if (ref == 2) count = xs_debugger_get_globals(dbg, vars, 32);
            for (int i = 0; i < count; i++) {
                char vbuf[256];
            ...
            snprintf(body, sizeof(body),
                "{\"result\":\"%s\",\"variablesReference\":0}", vbuf);

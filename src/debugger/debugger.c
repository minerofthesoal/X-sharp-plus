...
static void update_location(XsDebugger* dbg) {
    ...
    if (loc) {
        if (loc->file)
            strncpy(dbg->current_file, loc->file, XS_MAX_FILE_PATH - 1);
...
bool xs_debugger_execute_one(XsDebugger* dbg) {
    ...
    switch (op) {
    case 0x00: /* OP_PUSH_BLADE */
        dbg->vm.ip += 2;
        dbg->vm.stack_top++;
        break;
    case 0x01: /* OP_PUSH_SPARK */
        dbg->vm.ip += 2;
        dbg->vm.stack_top++;
        break;
    case 0x06: /* OP_POP */
        if (dbg->vm.stack_top > 0)
            dbg->vm.stack_top--;
        break;
    case 0x4A: /* OP_HALT */
        dbg->vm.halted = true;
...
bool xs_debugger_continue(XsDebugger* dbg) {
    ...
    while (dbg->state == DBG_STATE_RUNNING) {
        if (!xs_debugger_execute_one(dbg))
            break;
    }
    return true;
}
...
int xs_debugger_set_breakpoint(XsDebugger* dbg, const char* file, int line) {
    ...
    if (dbg->debug_info) {
        ...
        if (bp)
            bp->bytecode_offset =
                xs_source_map_find_offset(&dbg->debug_info->source_map, file, line);
    }
    return id;
}
...
int xs_debugger_get_call_stack(XsDebugger* dbg, XsDbgCallFrame* out, int max) {
    ...
    if (count > 0)
        memcpy(out, dbg->call_stack, count * sizeof(XsDbgCallFrame));
    return count;
}

XsDbgCallFrame* xs_debugger_get_frame(XsDebugger* dbg, int frame_id) {
    if (frame_id < 0 || frame_id >= dbg->call_stack_depth)
        return NULL;
...
int xs_debugger_add_watch(XsDebugger* dbg, const char* expression) {
    if (dbg->watch_count >= XS_DBG_MAX_WATCH)
        return -1;
...
bool xs_debugger_eval_watch(XsDebugger* dbg, int watch_id) {
    for (int i = 0; i < dbg->watch_count; i++) {
        if (dbg->watches[i].id == watch_id) {
            bool ok;
            dbg->watches[i].last_value =
                xs_debugger_evaluate(dbg, dbg->watches[i].expression, 0, &ok);
...
void xs_debugger_value_to_string(XsValue val, char* buf, size_t buf_size) {
    switch (val.type) {
    case VAL_BLADE:
        snprintf(buf, buf_size, "%ld", (long)val.blade);
        break;
    case VAL_SPARK:
        snprintf(buf, buf_size, "%g", val.spark);
        break;
    case VAL_SCROLL:
        snprintf(buf, buf_size, "\"%s\"", val.scroll ? val.scroll : "");
        break;
    case VAL_FATE:
        snprintf(buf, buf_size, "%s", val.fate ? "truth" : "lies");
        break;
    case VAL_ABYSS:
        snprintf(buf, buf_size, "abyss");
        break;
    default:

...
void xs_source_map_add(XsSourceMap* map, uint32_t offset, const char* file, int line, int column) {
    if (map->count >= map->capacity) {
        ...
        XsSourceMapEntry* new_entries =
            (XsSourceMapEntry*)realloc(map->entries, sizeof(XsSourceMapEntry) * new_cap);
        if (!new_entries)
            return;
...
XsFuncDebugInfo* xs_debug_info_add_func(XsDebugInfo* info, const char* name,
    ...
    if (info->func_count >= info->func_capacity) {
        ...
        XsFuncDebugInfo* new_funcs =
            (XsFuncDebugInfo*)realloc(info->functions, sizeof(XsFuncDebugInfo) * new_cap);
        if (!new_funcs)
            return NULL;
...
void xs_func_debug_add_param(XsFuncDebugInfo* func, const char* name, const char* type) {
    ...
    func->param_names = (char**)realloc(func->param_names, sizeof(char*) * func->param_count);
    func->param_types = (char**)realloc(func->param_types, sizeof(char*) * func->param_count);
...
void xs_func_debug_add_local(XsFuncDebugInfo* func, const char* name, const char* type, int slot,
    ...
    if (func->local_count >= func->locals_capacity) {
        ...
        XsVarDebugInfo* new_locals =
            (XsVarDebugInfo*)realloc(func->locals, sizeof(XsVarDebugInfo) * new_cap);
        if (!new_locals)
            return;
...
XsFuncDebugInfo* xs_debug_info_find_func(XsDebugInfo* info, uint32_t offset) {
    ...
    for (int i = 0; i < info->func_count; i++) {
        XsFuncDebugInfo* f = &info->functions[i];
        if (offset >= f->bytecode_start && offset < f->bytecode_start + f->bytecode_length) {
            return f;
        }
    }
    return NULL;
...
void xs_debug_info_add_global(XsDebugInfo* info, const char* name, const char* type, int slot) {
    ...
    if (info->global_count >= info->global_capacity) {
        ...
        XsVarDebugInfo* new_globals =
            (XsVarDebugInfo*)realloc(info->globals, sizeof(XsVarDebugInfo) * new_cap);
        if (!new_globals)
            return;
...
const char* xs_debug_info_get_line(XsDebugInfo* info, int line, int* out_length) {
    if (!info || !info->source_text || line < 1)
        return NULL;
...
static bool write_string(uint8_t** buf, size_t* size, size_t* cap, const char* s) {
...
bool xs_debug_info_serialize(XsDebugInfo* info, uint8_t** out_buf, size_t* out_size) {
    ...
    /* Magic */
    if (!write_u32(&buf, &size, &cap, 0x49445358))
        goto fail; /* "XSDI" */
    if (!write_u32(&buf, &size, &cap, 1))
        goto fail; /* version */

    if (!write_string(&buf, &size, &cap, info->module_name))
        goto fail;
    if (!write_string(&buf, &size, &cap, info->source_file))
        goto fail;
    if (!write_string(&buf, &size, &cap, info->source_text))
        goto fail;

    /* Source map */
    if (!write_u32(&buf, &size, &cap, (uint32_t)info->source_map.count))
        goto fail;
    for (int i = 0; i < info->source_map.count; i++) {
        XsSourceMapEntry* e = &info->source_map.entries[i];
        if (!write_u32(&buf, &size, &cap, e->bytecode_offset))
            goto fail;
        if (!write_i32(&buf, &size, &cap, e->loc.line))
            goto fail;
        if (!write_i32(&buf, &size, &cap, e->loc.column))
            goto fail;
    }

    /* Functions */
    if (!write_u32(&buf, &size, &cap, (uint32_t)info->func_count))
        goto fail;
    for (int i = 0; i < info->func_count; i++) {
        XsFuncDebugInfo* f = &info->functions[i];
        if (!write_string(&buf, &size, &cap, f->name))
            goto fail;
        if (!write_string(&buf, &size, &cap, f->source_file))
            goto fail;
        if (!write_i32(&buf, &size, &cap, f->line_start))
            goto fail;
        if (!write_i32(&buf, &size, &cap, f->line_end))
            goto fail;
        if (!write_u32(&buf, &size, &cap, f->bytecode_start))
            goto fail;
        if (!write_u32(&buf, &size, &cap, f->bytecode_length))
            goto fail;

        if (!write_u32(&buf, &size, &cap, (uint32_t)f->param_count))
            goto fail;
        for (int j = 0; j < f->param_count; j++) {
            if (!write_string(&buf, &size, &cap, f->param_names[j]))
                goto fail;
            if (!write_string(&buf, &size, &cap, f->param_types[j]))
                goto fail;
        }

        if (!write_u32(&buf, &size, &cap, (uint32_t)f->local_count))
            goto fail;
        for (int j = 0; j < f->local_count; j++) {
            XsVarDebugInfo* v = &f->locals[j];
            if (!write_string(&buf, &size, &cap, v->name))
                goto fail;
            if (!write_string(&buf, &size, &cap, v->type_name))
                goto fail;
            if (!write_i32(&buf, &size, &cap, v->slot))
                goto fail;
            if (!write_i32(&buf, &size, &cap, v->start_offset))
                goto fail;
            if (!write_i32(&buf, &size, &cap, v->end_offset))
                goto fail;
            if (!write_i32(&buf, &size, &cap, v->line_defined))
                goto fail;
        }
    }

    /* Globals */
    if (!write_u32(&buf, &size, &cap, (uint32_t)info->global_count))
        goto fail;
    for (int i = 0; i < info->global_count; i++) {
...
XsDebugInfo* xs_debug_info_deserialize(const uint8_t* buf, size_t size) {
    ...
    /* Source map */
    uint32_t sm_count;
    if (!read_u32(&p, end, &sm_count))
        goto fail;
    for (uint32_t i = 0; i < sm_count; i++) {
        uint32_t offset;
        int32_t line, col;
        if (!read_u32(&p, end, &offset))
            goto fail;
        if (!read_i32(&p, end, &line))
            goto fail;
        if (!read_i32(&p, end, &col))
            goto fail;
    ...
    for (uint32_t i = 0; i < func_count; i++) {
        ...
        if (!read_i32(&p, end, &ls)) {
            free(fname);
            free(ffile);
            goto fail;
        }
        if (!read_i32(&p, end, &le)) {
            free(fname);
            free(ffile);
            goto fail;
        }
        if (!read_u32(&p, end, &bc_start)) {
            free(fname);
            free(ffile);
            goto fail;
        }
        if (!read_u32(&p, end, &bc_len)) {
            free(fname);
            free(ffile);
            goto fail;
        }
        ...
        uint32_t lc;
        if (!read_u32(&p, end, &lc))
            goto fail;
        for (uint32_t j = 0; j < lc; j++) {
            char* ln = read_string(&p, end);
            char* lt = read_string(&p, end);
            int32_t slot, so, eo, ld;
            if (!read_i32(&p, end, &slot)) {
                free(ln);
                free(lt);
                goto fail;
            }
            if (!read_i32(&p, end, &so)) {
                free(ln);
                free(lt);
                goto fail;
            }
            if (!read_i32(&p, end, &eo)) {
                free(ln);
                free(lt);
                goto fail;
            }
            if (!read_i32(&p, end, &ld)) {
                free(ln);
                free(lt);
                goto fail;
            }

...
static bool is_binary_operator(TokenType t) {
    return (t == TOK_PLUS || t == TOK_MINUS || t == TOK_STAR || t == TOK_SLASH ||
            t == TOK_PERCENT || t == TOK_POWER || t == TOK_EQUAL_EQUAL || t == TOK_BANG_EQUAL ||
            t == TOK_LESS || t == TOK_GREATER || t == TOK_LESS_EQUAL || t == TOK_GREATER_EQUAL ||
            t == TOK_AND_AND || t == TOK_OR_OR || t == TOK_AMPERSAND || t == TOK_PIPE ||
            t == TOK_CARET || t == TOK_LSHIFT || t == TOK_RSHIFT || t == TOK_PIPE_GREATER ||
            t == TOK_DOT_DOT);
}

static bool is_assignment_operator(TokenType t) {
    return (t == TOK_EQUAL || t == TOK_PLUS_EQUAL || t == TOK_MINUS_EQUAL || t == TOK_STAR_EQUAL ||
            t == TOK_SLASH_EQUAL || t == TOK_PERCENT_EQUAL || t == TOK_POWER_EQUAL ||
            t == TOK_AMPERSAND_EQUAL || t == TOK_PIPE_EQUAL || t == TOK_CARET_EQUAL ||
            t == TOK_LSHIFT_EQUAL || t == TOK_RSHIFT_EQUAL);
}
...
static bool needs_space_before(TokenType prev, TokenType cur) {
    ...
    if (prev == TOK_LPAREN || prev == TOK_LBRACKET)
        return false;
    if (cur == TOK_RPAREN || cur == TOK_RBRACKET)
        return false;

    /* No space before comma, semicolon, colon */
    if (cur == TOK_COMMA || cur == TOK_SEMICOLON)
        return false;

    /* No space around member access dot */
    if (prev == TOK_DOT || cur == TOK_DOT)
        return false;

    /* No space between identifier and open paren (function call) */
    if ((prev == TOK_IDENTIFIER || prev == TOK_RPAREN) && cur == TOK_LPAREN)
        return false;

    /* No space between identifier and open bracket (indexing) */
    if ((prev == TOK_IDENTIFIER || prev == TOK_RPAREN) && cur == TOK_LBRACKET)
        return false;

    /* No space after unary operators */
    if (prev == TOK_BANG || prev == TOK_TILDE)
        return false;

    /* No space around :: */
    if (prev == TOK_DOUBLE_COLON || cur == TOK_DOUBLE_COLON)
        return false;

    /* No space before increment/decrement */
    if (cur == TOK_INCREMENT || cur == TOK_DECREMENT)
        return false;

    /* Space around binary operators */
    if (is_binary_operator(cur) || is_binary_operator(prev))
        return true;

    /* Space around assignment operators */
    if (is_assignment_operator(cur) || is_assignment_operator(prev))
        return true;
    ...
    if (prev == TOK_RBRACE && cur != TOK_SEMICOLON && cur != TOK_COMMA)
        return true;

    /* Space between arrow tokens and surrounding */
    if (prev == TOK_ARROW || cur == TOK_ARROW)
        return true;
    if (prev == TOK_FAT_ARROW || cur == TOK_FAT_ARROW)
        return true;

    /* Space between identifier and keyword */
    if (prev == TOK_IDENTIFIER && is_keyword_token(cur))
        return true;

    /* Default: space between most tokens */
    if (prev == TOK_IDENTIFIER && cur == TOK_IDENTIFIER)
        return true;
    if (prev == TOK_INT_LITERAL && cur == TOK_IDENTIFIER)
        return true;
...
char* xs_format_string(const char* source, const XsFormatConfig* config) {
    ...
    for (int i = 0; i < token_count; i++) {
        ...
        if (tok->line > prev_line) {
            ...
            if (blank_lines > 2)
                blank_lines = 2; /* max 2 blank lines */
        ...
        if (tok->type == TOK_RBRACE) {
            ...
            if (i + 1 < token_count) {
                ...
                next_continues = (nt == TOK_OTHERWISE || nt == TOK_DEFLECT || nt == TOK_SEMICOLON ||
                                  nt == TOK_COMMA);
            }
            if (!next_continues) {
                buf_append_char(&out, '\n');

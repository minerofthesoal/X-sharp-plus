...
static SymbolEntry *sym_add(SymbolTable *tbl, const char *name, int line, int col) {
    if (tbl->count >= MAX_SYMBOLS) return NULL;
...
static void result_add(XsLintResult *r, XsLintSeverity sev,
    ...
    if (sev == LINT_ERROR) r->error_count++;
    else if (sev == LINT_WARNING) r->warning_count++;
}
...
static bool is_camel_case(const char *name) {
    ...
    if (!islower((unsigned char)name[0]) && name[0] != '_') return false;
...
static void lint_walk(const AstNode *node, SymbolTable *syms,
                      XsLintResult *result, const char *file,
                      bool in_forge, bool forge_has_return);

static void lint_walk_list(const AstNodeList *list, SymbolTable *syms,
                           XsLintResult *result, const char *file,
                           bool in_forge, bool *has_return) {
    ...
    switch (node->type) {
            ...
            if (name) {
                ...
                if (!is_camel_case(name)) {
                    result_add(result, LINT_WARNING, file, node->line, node->column,
                               "naming-convention",
                               "Variable '%s' should use camelCase", name);
                }
            }
        ...
        case NODE_FORGE_DECL: {
            ...
            if (name) {
                ...
                if (!is_camel_case(name)) {
                    result_add(result, LINT_WARNING, file, node->line, node->column,
                               "naming-convention",
                               "Forge name '%s' should use camelCase", name);
                }
            }
            ...
            if (node->as.forge.body) {
                if (node->as.forge.body->type == NODE_BLOCK_STMT) {
                    lint_walk_list(&node->as.forge.body->as.block.statements,
                                   syms, result, file, true, &has_return);
                } else {
            ...
            if (node->as.forge.return_type.name &&
                strcmp(node->as.forge.return_type.name, "void") != 0 &&
                !has_return) {
                result_add(result, LINT_WARNING, file, node->line, node->column,
                           "missing-return",
                           "Forge '%s' has return type '%s' but may not return a value",
                           name ? name : "<anonymous>",
                           node->as.forge.return_type.name);
            }
            break;
        }
        ...
        case NODE_ENTITY_DECL: {
            ...
            if (name) {
                ...
                if (!is_pascal_case(name)) {
                    result_add(result, LINT_WARNING, file, node->line, node->column,
                               "naming-convention",
                               "Entity name '%s' should use PascalCase", name);
                }
            }
            bool hr = false;
        ...
        case NODE_CYCLE_IN_STMT: {
            ...
            if (node->as.cycle_in.var_name) {
                SymbolEntry *e = sym_add(syms, node->as.cycle_in.var_name,
        ...
        case NODE_SHIELD_STMT: {
            ...
            if (node->as.shield.catch_var) {
                SymbolEntry *e = sym_add(syms, node->as.shield.catch_var,
                                         node->line, node->column);
                if (e) {
                    e->is_declared = true;
                }
        ...
        case NODE_ARSENAL_LITERAL: {
            for (int i = 0; i < node->as.arsenal_literal.elements.count; i++) {
                lint_walk(node->as.arsenal_literal.elements.items[i],
                          syms, result, file, in_forge, false);
            }
            break;
        }
        ...
        case NODE_INTERPOLATED_STRING: {
            for (int i = 0; i < node->as.interp_string.parts.count; i++) {
                lint_walk(node->as.interp_string.parts.items[i],
                          syms, result, file, in_forge, false);
            }
            break;
        }
...
XsLintResult *xs_lint_string(const char *source, const char *filename) {
    ...
    if (lexer.had_error) {
        result_add(result, LINT_ERROR, filename, lexer.line, lexer.column,
                   "syntax-error", "Lexer error: %s", lexer.error_msg);
        free(tokens);
        return result;
    }
    ...
    if (parser_had_error(&parser)) {
        for (int i = 0; i < parser.error_count; i++) {
            result_add(result, LINT_ERROR, filename, 0, 0,
                       "syntax-error", "%s", parser.errors[i]);
        }
        ast_free(program);
        free(tokens);
        return result;
    ...
    for (int i = 0; i < syms.count; i++) {
        ...
        if (e->is_declared && !e->is_referenced &&
            !e->is_forge && !e->is_entity && !e->is_parameter) {
            result_add(result, LINT_WARNING, filename, e->line, e->column,
                       "unused-variable",
                       "Variable '%s' is declared but never used", e->name);
        }
        if (e->is_declared && !e->is_referenced && e->is_parameter) {
            result_add(result, LINT_HINT, filename, e->line, e->column,
                       "unused-parameter",
...
XsLintResult *xs_lint_file(const char *path) {
    ...
    if (!f) {
        ...
        result_add(r, LINT_ERROR, path, 0, 0, "file-error",
...
void xs_lint_result_print(const XsLintResult *result) {
    ...
    for (int i = 0; i < result->count; i++) {
        ...
        switch (d->severity) {
            case LINT_ERROR:   sev_str = "error";   sev_color = CLR_RED;    break;
            case LINT_WARNING: sev_str = "warning"; sev_color = CLR_YELLOW; break;
            case LINT_INFO:    sev_str = "info";    sev_color = CLR_CYAN;   break;
            case LINT_HINT:    sev_str = "hint";    sev_color = CLR_DIM;    break;
            default:           sev_str = "?";       sev_color = CLR_RESET;  break;
        }

        fprintf(stderr, "%s%s:%d:%d%s %s%s%s%s [%s] %s\n",
                CLR_BOLD, d->file, d->line, d->column, CLR_RESET,
                CLR_BOLD, sev_color, sev_str, CLR_RESET,
                d->rule, d->message);
    }
}

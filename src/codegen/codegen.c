...
static void ensure_code_capacity(Chunk *chunk, int extra) {
    ...
    if (needed <= chunk->code_capacity) return;
...
static void ensure_line_capacity(Chunk *chunk, int extra) {
    ...
    if (needed <= chunk->line_capacity) return;
...
int chunk_add_constant(Chunk *chunk, XsValue value) {
    ...
    for (int i = 0; i < chunk->const_count; i++) {
        ...
        if (existing.type == value.type) {
            switch (value.type) {
                case VAL_BLADE:
                    if (existing.blade == value.blade) return i;
                    break;
                case VAL_SPARK:
                    if (existing.spark == value.spark) return i;
                    break;
                case VAL_SCROLL:
                    if (existing.scroll && value.scroll &&
                        strcmp(existing.scroll, value.scroll) == 0) return i;
                    break;
                case VAL_FATE:
                    if (existing.fate == value.fate) return i;
                    break;
                case VAL_ABYSS:
                    return i;
                default:
...
int chunk_get_line(Chunk *chunk, int offset) {
    if (offset < 0 || offset >= chunk->line_count) return 0;
...
static void print_value(XsValue val) {
    switch (val.type) {
        case VAL_BLADE:  printf("%lld", (long long)val.blade); break;
        case VAL_SPARK:  printf("%g", val.spark); break;
        case VAL_SCROLL: printf("\"%s\"", val.scroll ? val.scroll : "(null)"); break;
        case VAL_FATE:   printf("%s", val.fate ? "truth" : "lies"); break;
        case VAL_ABYSS:  printf("abyss"); break;
...
static int invoke_instruction(const char *name, Chunk *chunk, int offset) {
    uint16_t name_idx = (uint16_t)(chunk->code[offset + 1] << 8) |
                        chunk->code[offset + 2];
...
int chunk_disassemble_instruction(Chunk *chunk, int offset) {
    ...
    switch ((OpCode)instruction) {
        case OP_PUSH_BLADE:    return constant_instruction("OP_PUSH_BLADE", chunk, offset);
        case OP_PUSH_SPARK:    return constant_instruction("OP_PUSH_SPARK", chunk, offset);
        case OP_PUSH_SCROLL:   return constant_instruction("OP_PUSH_SCROLL", chunk, offset);
        case OP_PUSH_TRUTH:    return simple_instruction("OP_PUSH_TRUTH", offset);
        case OP_PUSH_LIES:     return simple_instruction("OP_PUSH_LIES", offset);
        case OP_PUSH_ABYSS:    return simple_instruction("OP_PUSH_ABYSS", offset);
        ...
        case OP_AND:           return simple_instruction("OP_AND", offset);
        case OP_OR:            return simple_instruction("OP_OR", offset);
        ...
        case OP_LOAD_LOCAL:    return short_instruction("OP_LOAD_LOCAL", chunk, offset);
        case OP_STORE_LOCAL:   return short_instruction("OP_STORE_LOCAL", chunk, offset);
        ...
        case OP_LOAD_UPVALUE:  return short_instruction("OP_LOAD_UPVALUE", chunk, offset);
        case OP_STORE_UPVALUE: return short_instruction("OP_STORE_UPVALUE", chunk, offset);
        ...
        case OP_NEW:           return invoke_instruction("OP_NEW", chunk, offset);
        ...
        case OP_GET_METHOD:    return constant_instruction("OP_GET_METHOD", chunk, offset);
        case OP_INVOKE:        return invoke_instruction("OP_INVOKE", chunk, offset);

        case OP_NEW_ARSENAL:   return short_instruction("OP_NEW_ARSENAL", chunk, offset);
        ...
        case OP_ARSENAL_PUSH:  return simple_instruction("OP_ARSENAL_PUSH", offset);

        case OP_GPU_DISPATCH:  return simple_instruction("OP_GPU_DISPATCH", offset);
        case OP_GPU_SYNC:      return simple_instruction("OP_GPU_SYNC", offset);

        case OP_ENGRAVE:       return byte_instruction("OP_ENGRAVE", chunk, offset);
        case OP_PIPE:          return simple_instruction("OP_PIPE", offset);

        case OP_INC:           return simple_instruction("OP_INC", offset);
        case OP_DEC:           return simple_instruction("OP_DEC", offset);

        case OP_CAST:          return byte_instruction("OP_CAST", chunk, offset);
        case OP_RANGE:         return simple_instruction("OP_RANGE", offset);
        case OP_SPREAD:        return simple_instruction("OP_SPREAD", offset);

        case OP_SHIELD_BEGIN:  return jump_instruction("OP_SHIELD_BEGIN", 1, chunk, offset);
        case OP_SHIELD_END:    return simple_instruction("OP_SHIELD_END", offset);
        case OP_SHATTER:       return simple_instruction("OP_SHATTER", offset);

...
void ast_print(const AstNode *node, int indent) {
    ...
    switch (node->type) {
        ...
        case NODE_CONJURE_EXPR:
            printf(" %s (%d args)\n", node->as.conjure.entity_name,
                   node->as.conjure.args.count);
        ...
        case NODE_FORGE_DECL:
            printf(" %s (%d params) -> ", node->as.forge.name,
                   node->as.forge.params.count);
        ...
        case NODE_SUMMON_DECL:
            ...
            if (node->as.summon.alias) printf(" as %s", node->as.summon.alias);
            printf("\n");
            break;

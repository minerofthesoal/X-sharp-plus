...
static Token error_token(Lexer *lexer, const char *msg) {
    ...
    tok.literal.int_val = 0;
    snprintf(lexer->error_msg, LEXER_MAX_ERROR, "Line %d, Col %d: %s",
...
static Token scan_string(Lexer *lexer) {
    ...
    while (!lexer_is_at_end(lexer)) {
        ...
        if (c == '\\') {
            ...
            if (!lexer_is_at_end(lexer)) advance(lexer); /* escaped char */
            continue;
        }
...
static Token scan_string_continuation(Lexer *lexer) {
    ...
    while (!lexer_is_at_end(lexer)) {
        ...
        if (c == '\\') {
            advance(lexer);
            if (!lexer_is_at_end(lexer)) advance(lexer);
            continue;
        }
        if (c == '#' && peek_next(lexer) == '{') {
...
static Token scan_identifier(Lexer *lexer) {
    ...
    for (const Keyword *kw = keywords; kw->word != NULL; kw++) {
        if (strlen(kw->word) == len &&
            memcmp(kw->word, lexer->token_start, len) == 0) {
...
Token lexer_next_token(Lexer *lexer) {
    ...
    if (c == '@') {
        ...
        if (peek(lexer) == 'a' && lexer->current[1] == 'i' &&
            !isalnum(lexer->current[2]) && lexer->current[2] != '_') {
            advance(lexer); advance(lexer);
    ...
    switch (c) {
        ...
        case '#':  return make_token(lexer, TOK_HASH);
        case '~':
        ...
        case ':':
            if (match(lexer, ':')) return make_token(lexer, TOK_DOUBLE_COLON);
            return make_token(lexer, TOK_COLON);

        case '+':
            if (match(lexer, '+')) return make_token(lexer, TOK_INCREMENT);
            if (match(lexer, '=')) return make_token(lexer, TOK_PLUS_EQUAL);
            return make_token(lexer, TOK_PLUS);

        case '-':
            if (match(lexer, '-')) return make_token(lexer, TOK_DECREMENT);
            if (match(lexer, '=')) return make_token(lexer, TOK_MINUS_EQUAL);
            if (match(lexer, '>')) return make_token(lexer, TOK_ARROW);
            return make_token(lexer, TOK_MINUS);

        case '*':
            if (match(lexer, '*')) {
                if (match(lexer, '=')) return make_token(lexer, TOK_POWER_EQUAL);
                return make_token(lexer, TOK_POWER);
            }
            if (match(lexer, '=')) return make_token(lexer, TOK_STAR_EQUAL);
            return make_token(lexer, TOK_STAR);

        case '/':
            if (match(lexer, '=')) return make_token(lexer, TOK_SLASH_EQUAL);
            return make_token(lexer, TOK_SLASH);

        case '%':
            if (match(lexer, '=')) return make_token(lexer, TOK_PERCENT_EQUAL);
            return make_token(lexer, TOK_PERCENT);

        case '=':
            if (match(lexer, '=')) return make_token(lexer, TOK_EQUAL_EQUAL);
            if (match(lexer, '>')) return make_token(lexer, TOK_FAT_ARROW);
            return make_token(lexer, TOK_EQUAL);

        case '!':
            if (match(lexer, '=')) return make_token(lexer, TOK_BANG_EQUAL);
            return make_token(lexer, TOK_BANG);

        case '<':
            if (match(lexer, '<')) {
                if (match(lexer, '=')) return make_token(lexer, TOK_LSHIFT_EQUAL);
                return make_token(lexer, TOK_LSHIFT);
            }
            if (match(lexer, '=')) return make_token(lexer, TOK_LESS_EQUAL);
            return make_token(lexer, TOK_LESS);

        case '>':
            if (match(lexer, '>')) {
                if (match(lexer, '=')) return make_token(lexer, TOK_RSHIFT_EQUAL);
                return make_token(lexer, TOK_RSHIFT);
            }
            if (match(lexer, '=')) return make_token(lexer, TOK_GREATER_EQUAL);
            return make_token(lexer, TOK_GREATER);

        case '&':
            if (match(lexer, '&')) return make_token(lexer, TOK_AND_AND);
            if (match(lexer, '=')) return make_token(lexer, TOK_AMPERSAND_EQUAL);
            return make_token(lexer, TOK_AMPERSAND);

        case '|':
            if (match(lexer, '|')) return make_token(lexer, TOK_OR_OR);
            if (match(lexer, '>')) return make_token(lexer, TOK_PIPE_GREATER);
            if (match(lexer, '=')) return make_token(lexer, TOK_PIPE_EQUAL);
            return make_token(lexer, TOK_PIPE);

        case '^':
            if (match(lexer, '=')) return make_token(lexer, TOK_CARET_EQUAL);
...
Token *lexer_tokenize_all(Lexer *lexer, int *count) {
    ...
    for (;;) {
        if (n >= capacity) {
            ...
            if (!tmp) { free(tokens); *count = 0; return NULL; }
            tokens = tmp;
        }
